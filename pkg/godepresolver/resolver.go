...
func (r *GoDepResolver) RecursivelyResolveImportsToCopyCommands(pkgPath string) (imports []spec.Command, testOnlyImports []spec.Command, err error) {
	err = filepath.Walk(pkgPath, func(p string, inf fs.FileInfo, err error) error {
		...
		i, t, err := r.ResolveImportsToCopyCommands(p)
		if err != nil {
			return fmt.Errorf("could not resolve imports for pkg %q: %v\n", p, err)
		}
...
func (r *GoDepResolver) ResolveImportsToCopyCommands(pkgPath string) (imports []spec.Command, testOnlyImports []spec.Command, err error) {
...
func (r *GoDepResolver) resolveCopyCommandsForImports(pkgImports mapset.Set[string], pkgPathAbs string) ([]spec.Command, error) {
	...
	for imp := range pkgImports.Iter() {
		impDir := strings.Replace(imp, modPath, r.goModRoot, 1)
